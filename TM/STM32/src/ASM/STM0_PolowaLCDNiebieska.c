...
void fillMemory(void* adres_bazowy, unsigned long rozmiar, unsigned short int kolor);
...
int main(void) {
	...
	while (1) {
		...
		fillMemory((void*)LCD_BUF, 240*160, 0x1f); //polowa wyswietlacza = 240*160
		...
		for (int yy = 0; yy < 32; yy++) {
			unsigned short int *pixel = (unsigned short int*) (LCD_BUF + x
					+ (y + yy) * 240);
